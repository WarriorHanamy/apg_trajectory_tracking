...
class CartPoleEnv:
    ...
    def _step(
        ...
        if self.is_img_dyn:
            ...
            next_torch_state = self.dynamics(torch_state.float(), action, dt=self.dt)
    ...
    def _reset_swingup(self) -> FloatArray:
        ...
        self.state[2] = rand_sign * (2.8 + np.random.rand() * 0.3)
        self.state[3] *= 0.1
        return self.state
    ...
    def _reset_upright(self) -> FloatArray:
        ...
        self.state = (np.random.rand(4) - 0.5) * 0.3
...
def construct_states(
    ...
    **kwargs,
    ...
    while len(data) < num_data:
        ...
        env.state = (np.random.rand(4) - 0.5) * 0.1
...
def make_state_to_img_dataset(dataset_size: int = 2000) -> None:
    ...
    for i in range(dataset_size):
        ...
        img = preprocess_img(env._render(mode="rgb_array"), 300, 200)[
            75:175, img_width_base - img_des_half : img_width_base + img_des_half
        ]
